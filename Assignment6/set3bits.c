...
#include <stdio.h>
#include <math.h>
unsigned char set3bits(
    unsigned char c, int firstbit, int secondbit, int thirdbit);
int main() {
    unsigned char c, bitmask, setBit;
    int i, d, firstbit, secondbit, thirdbit;
    d = 0;
    ...
    //Fucntion call
    setBit = set3bits(c, firstbit, secondbit, thirdbit);
    bitmask = setBit;
...
unsigned char set3bits(
    ...
    // shift left 1 by the declared arguments and
    // use bitwise or operator to turn on the bit
    c = (c | (1 << firstbit));
    c = (c | (1 << secondbit));
    c = (c | (1 << thirdbit));
    return c;
